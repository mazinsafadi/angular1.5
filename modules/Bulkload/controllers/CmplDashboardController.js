(function () {
    'use strict';
    angular.module('Bulkload.CmplDashboardController', []).controller('CmplDashboardController', CmplDashboardController);

    function CmplDashboardController($scope, $modal, $window, $timeout, $compile, bulkloadServices,$rootScope,$http,url,$base64,$state) {
        var vm = this;
        $scope.userid="";
    	    ...
    	    function handleDocHeightMsg(e) {
    	        ...
    	        if ( e.origin === e.data.origin ) {
    	            ...
    	            if(data.height){
    	            	...
    	            	$scope.maxHeight=$('#contentPannel').height()-($('.displayData-btn').height()+$('.topTitle').height()+55+75+18);
    	            	$('table.dashboardTable tbody').css('max-height',$scope.maxHeight+'px');
    	            	$('table.accTable tbody').css('max-height',$scope.maxHeight+'px');
    	            	$('table.dashboardTable tbody').css('height',$scope.maxHeight+'px');
    	            	$('table.accTable tbody').css('height',$scope.maxHeight+'px');
        ...
        $scope.currentPage = 0;
         $scope.loadDashboard=function(start,stop,isRefresh){
        	 ...
        	 if(isRefresh){
        		 $scope.currentPage = 0;
        		 //$scope.dat=moment().tz("America/New_York").format();
        		 var format = 'DD/MM/YYYY HH:mm';
        		 $scope.dat=moment(new Date(), format).tz('America/New_York').format(format);
        		 $scope.search={};
        	 }
    	 ...
    	 bulkloadServices.getCmplDashboard($scope.loadParams).then(function(data){
		       	$scope.CmplFiles=data;
		       	if(data.length!==0){
            ...
            $scope.searchCC=function(start,stop){
            	var format = 'DD/MM/YYYY HH:mm';
        		 $scope.dat=moment(new Date(), format).tz('America/New_York').format(format);
            	if(start==1){
            		$scope.currentPage=0;
            	...
            	$rootScope.searchbuttonclicked=true;
            	$scope.attrs={
            		'startRecords':$scope.start,
            		'endRecords':$scope.stop,
            		'fileName':$scope.search.fileName,
            		'reqId':$scope.search.reqId,
            		'status':$scope.search.status,
            		'uploadBy':$scope.search.uploadedBy
            	};
            	bulkloadServices.searchCCDashboardFile($scope.attrs).then(function(data){
            			$scope.CmplFiles=data;
		       	if(data.length!==0){
		        $scope.total=data[0].totalFileCount;
		        $scope.init();
				...
				$scope.range = function() {
		            ...
		            $scope.prevPage = function() {
		              if ($scope.currentPage > 0) {
		                $scope.currentPage--;
		                $rootScope.searchbuttonclicked=true;
		                $scope.searchCC($scope.currentPage*20+1,$scope.currentPage*20+20);
		              }
		            };
		            $scope.prevPageDisabled = function() {
		              return $scope.currentPage === 0 ? 'disabled' : '';
            ...
            $scope.searchCCAcc=function(start,stop){
            	var format = 'DD/MM/YYYY HH:mm';
        		 $scope.dat=moment(new Date(), format).tz('America/New_York').format(format);
            	...
            	bulkloadServices.searchCCDashboardAcc($scope.attrs).then(function(data){
            			$scope.CCAccounts=data.bulkAccounts;
				...
				$scope.range = function() {
		            ...
		            $scope.prevPage = function() {
		              if ($scope.currentPage > 0) {
		                $scope.currentPage--;
		                $rootScope.searchbuttonclicked=true;
		                $scope.searchCCAcc($scope.currentPage*20+1,$scope.currentPage*20+20);
		              }
		            };
		            $scope.prevPageDisabled = function() {
		              return $scope.currentPage === 0 ? 'disabled' : '';
    ...
    CmplDashboardController.$inject = ["$scope", "$modal", "$window", "$timeout", "$compile", "bulkloadServices","$rootScope","$http","url","$base64","$state"];
})();
(function($) {
